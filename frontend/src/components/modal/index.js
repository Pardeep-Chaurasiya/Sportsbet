...
export default class AccModal extends React.Component {
  constructor(props) {
    ...
    this.state = {
      amount: "",
      claimData: { isCash: null },
      amountEmpty: !1,
      allowSlideNext: false,
      allowSlidePrev: false,
      depositWithdraw: { type: null, merchant: null, msg: null },
    };
  ...
  componentDidMount() {
    this.livePreviewSwiper = new Swiper(".swiper-container-modal", {
      noSwiping: false,
      allowSlideNext: this.state.allowSlideNext,
      allowSlidePrev: this.state.allowSlidePrev,
    });
    this.livePreviewSwiper.on("slideChange", this.handleSlideChange);
  }
  componentDidUpdate(prevProps) {
    prevProps.sb_modal.tabType !== this.props.sb_modal.tabType &&
      this.props.sb_modal.tabType !== 0 &&
      this.showNext();
  }
  ...
  closeModal() {
    this.livePreviewSwiper.slideReset();
    this.props.dispatch(
      allActionDucer(MODAL, { modalOpen: false, type: null })
    );
  }
  backToMenuModal() {
  ...
  changeTab(type, tabType = null) {
    ...
    this.props.dispatch(
      allActionDucer(MODAL, { type: type, tabType: hasType ? tabType : 0 })
    );
    if (hasType) {
      this.showNext();
    }
  ...
  claimDialog(dat) {
    let settable = {};
    dat.type === 1
      ? (settable.attempttingBonusClaim =
          !this.props.sb_modal.attempttingBonusClaim)
      : (settable.attempttingBonusWithdraw =
          !this.props.sb_modal.attempttingBonusWithdraw);
    this.props.dispatch(
      allActionDucer(MODAL, {
        showClaimDialog: !this.props.sb_modal.showClaimDialog,
        ...settable,
      })
    );
  ...
  depositSuccessDialog(data) {
    this.props.dispatch(
      allActionDucer(MODAL, {
        showDepositDialog: !this.props.sb_modal.showDepositDialog,
      })
    );
  ...
  withdrawSuccessDialog(data) {
    this.props.dispatch(
      allActionDucer(MODAL, {
        showWithdrawalDialog: !this.props.sb_modal.showWithdrawalDialog,
      })
    );
    this.setState({ depositWithdraw: data });
  }
  ...
  render() {
    const {
      ...
      { amount, depositWithdraw, claimData, amountEmpty } = this.state,
      { sportsbook, profile, sendRequest, dispatchLogout } = this.props,
      showStyles = { opacity: 1, pointerEvents: "unset" };

    return (
      <div
        className={`sb-modal sb-modal-backdrop`}
        style={modalOpen ? { ...showStyles } : {}}
      >
        <div
          className="sb-modal-backdrop"
          style={showClaimDialog ? { ...showStyles } : {}}
        >
            ...
            <div className="account-popup-body-container">
              {claimData.type === 2 && (
                <div className="account-popup-message">
                  {" "}
                  Early withdrawal bonuses can cause losses!{" "}
                  <a onClick={this.closeModal} href="/bonus-terms">
                    <span>Read Bonus Terms</span>
                  </a>
                </div>
              )}
              <div className="account-popup-message">
                Are you sure want to{" "}
                {claimData.type === 1 ? "claim this " : "withdraw "} bonus?
              </div>
              {claimData.isCash === 1 && (
                          ...
                          <div className="form-element empty">
                            <div className="input-wrapper  show-password-switcher">
                              <div className="field-icons-container ember-view">
                                {amountEmpty && (
                                  <React.Fragment>
                                    ...
                                    <div
                                      className="field-message-container ember-view"
                              ...
                              <input
                                ...
                                value={amount}
                                className={`${
                                  amountEmpty ? "error animated pulse" : ""
              ...
              <div className="account-popup-buttons">
                <a className="btn btn1" onClick={this.confirmClaim.bind(this)}>
                  Yes {claimData.type === 2 && " Withdraw"}
                </a>
                <a
                  className="btn btn1 cancel"
                  onClick={this.cancelClaim.bind(this)}
                >
                  Cancel
                </a>
              </div>
        ...
        <div
          className="sb-modal-backdrop"
          style={showDepositDialog ? { ...showStyles } : {}}
        >
          <div className="account-popup-container container-no-background">
            <div className="account-popup-header">
              <span
                className={`${
                  depositWithdraw?.status == 200
                    ? "icon-sb-success"
                    : "icon-sb-error"
                }`}
              ></span>
              <span style={{ padding: "0 10px" }}>
                {depositWithdraw?.status == 200 ? "Success" : "Falied"}
              </span>
            </div>

            <div className="account-popup-body-container">
              <div
                className="account-popup-message"
                style={{ fontWeight: "900", fontSize: "16px" }}
              >
                {depositWithdraw?.type !== 1
                  ? "Thank you for making a deposit"
                  : ""}
              </div>
              <div className="account-popup-message">
                {depositWithdraw?.type !== 1
                  ? `Please click Okay button or copy the URL to transfer the requested funds: `
                  : ""}
              </div>
              {depositWithdraw?.type !== 1 && (
                <div className="account-popup-message">
                  ...
                  >
                    ...
                    <span
                      ...
                      onClick={() => this.copyToClipboard(depositWithdraw.url)}
                    ></span>
                  </div>
                </div>
              )}
              {depositWithdraw?.type !== 1 && (
                <div className="account-popup-message">
                  Validation takes about 5-15 minutes depending of transaction
                  volume. We advice you to only contact Live chat if your
                  doposit does not reflect 15 minutes after after fund transfer
                </div>
              )}
              <div className="account-popup-buttons">
                ...
                <a
                  className="btn btn1 cancel"
                  onClick={() => {
                    this.openInNewTab(depositWithdraw.url);
                    this.cancelDepositDialog();
                  }}
                >
                  Okay
                </a>
        ...
        <div
          className="sb-modal-backdrop"
          style={showWithdrawalDialog ? { ...showStyles } : {}}
        >
          <div className="account-popup-container container-no-background">
            <div
              className="account-popup-header"
              style={{
                backgroundColor: depositWithdraw?.status !== 200 && "krimson",
              }}
            >
              <span
                className={`${
            ...
            <div className="account-popup-body-container">
              ...
              >
                {depositWithdraw?.status == 200
                  ...
                  : "Sorry Something is was not right"}
              </div>
              <div className="account-popup-message">{depositWithdraw.msg}</div>
              <div className="account-popup-buttons">
                {/* <a className="btn btn1" onClick={this.confirmClaim.bind(this)}>Yes {claimData.type ===2 &&' Withdraw'}</a> */}
                <a
                  className="btn btn1 cancel"
                  onClick={() => {
                    this.cancelWithdrawalDialog();
                  }}
                >
                  Okay
                </a>
        ...
        <div
          className="swiper-container col-sm-12 swiper-container-modal"
          style={{ height: "100%" }}
        >
              ...
              <div className="sb-modal-content">
                <div className="sb-modal-menu col-sm-12">
                  <div className="user-balance-section">
                    <div className="user">
                      <div
                        className="avatar-container col-sm-2"
                        style={{
                          display: "flex",
                      ...
                      <div className="number-id col-sm-10">
                        ...
                        <span className="id">
                          +{profile.userData?.dialing_code}
                          {profile.userData?.mobilenumber}
                        </span>
                  ...
                  <AccountMenuOption
                    type={type}
                    onExpand={this.changeTab}
                    id={1}
                    title={"Profile"}
                    icon={"icon-sb-edit-profile"}
                    tabType={tabType}
                  >
                    <div
                  ...
                  <AccountMenuOption
                    type={type}
                    onExpand={(id) => this.changeTab(id, 1)}
                    id={2}
                    title={"Bets History"}
                    icon={"icon-sb-my-bets"}
                    tabType={tabType}
                  />
              ...
              <div className="sb-modal-content">
                {type === 2 ? (
                  <BetHistory
                    ...
                    backToMenuModal={this.backToMenuModal}
                  />
                ) : type === 1 ? (
                  <UserProfile
                    backToMenuModal={this.backToMenuModal}
