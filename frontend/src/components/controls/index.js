...
export default class Controls extends React.Component {
  ...
  componentDidMount() {
    const { betSelections, betMode, sys_bet_variant } = this.props.sportsbook;
    if (Object.keys(betSelections).length === 1 && betMode !== 1)
      this.changeBetMode(1, true);
    if (
      Object.keys(betSelections).length > 1 &&
      Object.keys(betSelections).length < 3 &&
      betMode === 3
    )
  ...
  showMatchDetails(e, d) {
    ...
    this.setState({
      ...
      details: { match: d, top: top, left: left },
    });
  }
  hideMatchDetails() {
  ...
  validate() {
    if (this.searchTicketInput && this.recaptchaValue) {
      ...
      let val = this.searchTicketInput.value,
  ...
  changeBetMode(event, manual = false) {
    ...
    this.props.dispatch(
      allActionDucer(SPORTSBOOK_ANY, {
        betMode: manual ? event : parseInt(event.target.value),
        betStake: 0,
        bookingNumber: null,
        enableFreebet: false,
      })
    );
    this.props.sportsbook.sessionData.sid !== undefined &&
      isLoggedIn &&
  ...
  useQuickBet(e) {
    this.props.dispatch(
      allActionDucer(SPORTSBOOK_ANY, { isQuickBet: e.target.checked })
    );
  }
  useFreeBet(e) {
  ...
  setFreeBetStake(e) {
    let { freeBetStake, freeBets } = this.props.sportsbook,
      stake = e.target.value;
    if (freeBets[parseInt(stake)].id !== freeBetStake.id)
      this.props.dispatch(
        allActionDucer(SPORTSBOOK_ANY, {
          freeBetStake: freeBets[parseInt(stake)],
        })
      );
  }
  setSingleBetStake(gameId, event) {
    ...
    if (stake > 0) {
      if (betSelections[gameId]) {
        ...
        betSelections[gameId].singlePosWin =
          stake * betSelections[gameId].price;
      }
    } else {
      if (betSelections[gameId]) {
  ...
  onClearSingle(gameId) {
    const { betSelections, lowBalance, betSlipMode, betStake } =
      this.props.sportsbook;
    if (betSelections[gameId]) {
      let oldStake = betSelections[gameId].singleStake,
        stake = isNaN(oldStake) ? 0 : oldStake,
        stakeString = stake.toString();
  ...
  setSysBetVariant(event, manual = false) {
    this.props.dispatch(
      allActionDucer(SPORTSBOOK_ANY, {
        sys_bet_variant: manual ? event : JSON.parse(event.target.value),
      })
    );
  }
  checkForSelectionErrors(betSelections) {
    return Object.entries(betSelections).filter(function (value, index, arr) {
      return (
        value[1].conflicts.length > 0 ||
        (value[1].initialPrice != null &&
          value[1].price !== value[1].initialPrice) ||
        (value[1].suspended !== void 0 && value[1].suspended == 1)
      );
    });
  }
  ...
  betFailed(code, extra = null) {
    ...
    switch (code) {
      case 1800:
        ...
        if (extra)
          stateData.betSlipNotyMsg = stateData.betSlipNotyMsg
            .replace("%p", extra.old)
            .replace("%c", extra.new);
        break;
  ...
  placeBet() {
    ...
    if (Object.keys(betSelections).length > 0) {
      ...
      Object.keys(betSelections).forEach((selected) => {
        ...
        eventDetails["CompetitionId"] =
          betSelections[selected].gamePointer.competition;
        eventDetails["CompetitionName"] =
          betSelections[selected].gamePointer.competitionName;
        eventDetails["SportId"] = betSelections[selected].gamePointer.sport;
        eventDetails["SportName"] =
          betSelections[selected].gamePointer.sport_name;
        eventDetails["SportFullName"] =
          betSelections[selected].gamePointer.alias;
        eventDetails["Price"] = betSelections[selected].price;
        eventDetails["IsLive"] = betSelections[selected].gamePointer.isLive
          ? betSelections[selected].gamePointer.isLive
          : 0;
        eventDetails["Basis"] = betSelections[selected].eventBases || 0.0;
        ...
        eventDetails["MatchStartDate"] = moment
          .unix(betSelections[selected].start_ts)
          .format("YYYY-MM-DD H:mm:ss");
        eventDetails["EventEndDate"] = moment
          .unix(betSelections[selected].start_ts)
          .add(parseInt(betSelections[selected].match_length) + 20, "minutes")
          .format("YYYY-MM-DD H:mm:ss");
      ...
      if (errors.length) {
        ...
        let code, extra;
        if (firstError.suspended) code = 2036;
        else if (firstError.conflicts.length) code = 3019;
        else {
          code = 1800;
      ...
      if (selectionArr.length) {
        const userId = getCookie("id"),
          ...
          hash = calcMD5(
            `AuthToken${authToken}Amount${Number.parseFloat(betStake).toFixed(
              2
            )}BetType${betMode}TotalPrice${totalOdd.toFixed(
              3
            )}Uid${userId}Email${email}${this.props.appState.$betKey}`
          );
        let params = {
          command: "do_bet",
          // Hash: hash,
        ...
        if (selectionArr.length > 1 && betMode === 1) {
          selectionArr.map((sele) => {
            if (
              sele.singleStake >= config.min_bet_stakes[config.currency] ||
              enableFreebet
            ) {
              params.Amount = enableFreebet
                ? freeBetStake.amount
                : sele.singleStake.toFixed(2);
              ...
              if (enableFreebet) {
                params.bonus_id = freeBetStake.id;
                params.wallet_type = 2;
              }
              ...
              dispatch(
                allActionDucer(SPORTSBOOK_ANY, {
                  showBetSlipNoty: true,
                  betSlipNotyType: "warning",
                  betSlipNotyMsg:
                    "Bet Amount is less than minimum " +
                    config.min_bet_stakes[config.currency],
                  betInprogress: false,
                })
              );
              ...
              this.minBetStakesTimeout = setTimeout(() => {
                dispatch(
                  allActionDucer(SPORTSBOOK_ANY, { showBetSlipNoty: false })
                );
              }, 5000);
            }
            return true;
        ...
        if (enableFreebet) {
          params.Amount = freeBetStake.amount;
          params.bonus_id = freeBetStake.id;
          params.wallet_type = 2;
        }
        if (betMode === 3) {
          params.SystemMinCount = sys_bet_variant.variant;
          params.Amount = Number.parseFloat(
            (Number.parseFloat(betStake) * sys_bet_variant.bets).toFixed(2)
          ).toFixed(2);
        }
        this.rids[12].request = params;
        if (
          (betMode === 2 && totalOdd > config.max_odd_for_multiple_bet) ||
          selectionArr.length > config.max_selections_in_multiple_bet
        ) {
          if (totalOdd > config.max_odd_for_multiple_bet) {
            dispatch(
              allActionDucer(SPORTSBOOK_ANY, {
                ...
                betSlipNotyType: "warning",
                betSlipNotyMsg:
                  "Maximun odds for bet multiple bet (" +
                  config.max_odd_for_multiple_bet +
                  ") reached ",
                betInprogress: false,
              })
            );
          ...
          if (selectionArr.length > config.max_selections_in_multiple_bet) {
            dispatch(
              allActionDucer(SPORTSBOOK_ANY, {
                ...
                betSlipNotyType: "warning",
                betSlipNotyMsg:
                  "Maximum selection for Multiple bet is " +
                  config.max_selections_in_multiple_bet,
                betInprogress: false,
              })
            );
  ...
  bookBet() {
    let selectionArr = [],
      { betSelections, betMode, betStake, sys_bet_variant } =
        this.props.sportsbook;
    if (Object.keys(betSelections).length > 0) {
      this.props.dispatch(
        allActionDucer(SPORTSBOOK_ANY, { betInprogress: true })
      );
      Object.keys(betSelections).forEach((selected) => {
        selectionArr.push({
          event_id: betSelections[selected].eventId,
          price: betSelections[selected].price,
          marketId: betSelections[selected].marketId,
        });
      });
      if (selectionArr.length) {
        let params = {
        ...
        if (selectionArr.length > 1 && betMode === 1) {
          selectionArr.map((sele) => {
            ...
            let newRid = {};
            newRid[sele.marketId] = {
              rid: sele.marketId,
              callback: this.props.handleBetResponse,
              request: params,
            };
        ...
        if (betMode === 3) {
          ...
          params.params.amount = Number.parseFloat(
            betStake * sys_bet_variant.bets
          ).toFixed(2);
        }
  ...
  setBetStake(event) {
    const stake = Number(event.target.value).toFixed(2),
      {
        isQuickBet,
        betMode,
        betSlipMode,
    ...
    if (isQuickBet) {
      if (stake > 0) {
        stateData.quickBetStake = stake;
      } else stateData.quickBetStake = 0;
    } else {
      ...
      if (stake > 0) {
        if (betMode === 1) {
          Object.keys(betSelectionsCopy).forEach((key, index) => {
            ...
            betSelectionsCopy[key].singlePosWin =
              stateb * betSelectionsCopy[key].price;
          });
  ...
  deposit() {
    this.props.dispatch(
      allActionDucer(MODAL, { modalOpen: true, type: 3, tabType: 1 })
    );
  }
  ...
  reCalculate() {
    ...
    let d = [],
      ...
      g,
      e,
      c = sys_bet_variant.variant;
    for (e = 0; e < c; e++) {
      d[e] = e;
      h[e] = betSelectionsArr.length - e;
    }
    h = h.reverse();
    e = c - 1;
    for (let l; d[0] <= h[0]; )
      if (d[e] < h[e])
        if (e !== c - 1) e = c - 1;
          ...
          g = 1;
          for (l = 0; l < c; l++)
            0 === betSelectionsArr[d[l]].flag
              ? (g *= betSelectionsArr[d[l]].price)
              : 1 === betSelectionsArr[d[l]].flag && (g = 0);
          f = (100 * f + this.mathCuttingFunction(100 * g)) / 100;
          d[e]++;
        }
      else for (e--, d[e]++, g = e; g < c - 1; g++) d[g + 1] = d[g] + 1;
  ...
  removeAllBetSelections() {
    ...
    this.props.dispatch(
      allActionDucer(SPORTSBOOK_ANY, {
        betSelections: {},
        betMode: 1,
        betStake: isQuickBet ? betStake : 0,
      })
    );
    dataStorage("betSlip", {}, 2);
  ...
  removeGameFromBets(gameId) {
    const { betSelections, subscriptions, selectionSub } =
      this.props.sportsbook;
    let betSLen = Object.keys(betSelections).length;
    let stateData = {};
    if (betSLen === 1) {
  ...
  onOddsSettingsChange(e) {
    let acceptMode = this.props.sportsbook.acceptMode,
      mode = e.target.value;
    if (mode !== acceptMode)
      this.props.dispatch(
        allActionDucer(SPORTSBOOK_ANY, { acceptMode: parseInt(mode) })
      );
  }
  betslipToggleView() {
    const { showFirsttime } = this.props.sportsbook;
    this.setState((prevState) => ({
      isBetSlipOpen: showFirsttime ? false : !prevState.isBetSlipOpen,
    }));
    this.props.dispatch(
      allActionDucer(SPORTSBOOK_ANY, {
        showBetslipSettings: false,
        animation: false,
        showFirsttime: showFirsttime ? false : showFirsttime,
      })
    );
  }
  onOddsTypeChange(e) {
    let oddType = this.props.sportsbook.oddType,
      type = e.target.value;
    if (type !== oddType) {
  ...
  retrieve() {
    if (this.retrieveBookingInput && this.retrieveBookingInput.value !== "") {
      this.props.dispatch(
        allActionDucer(SPORTSBOOK_ANY, { retrieveBookingLoading: true })
      );
  ...
  getTotalBets() {
    let totalBets = 0,
      { betSelections, sys_bet_variant, enableFreebet, betMode } =
        this.props.sportsbook;
    if (betMode === 1) {
      Object.keys(betSelections).forEach((sele, ind) => {
        if (
          betSelections[sele].singleStake !== "" &&
          betSelections[sele].singleStake !== 0
        ) {
          totalBets += 1;
        }
      });
      ...
      return null !== sys_bet_variant
        ? sys_bet_variant.bets
        : this.sys_bet_result.options;
    } else if (enableFreebet) return Object.keys(betSelections).length;
  }
  getTotalStake() {
    let totalStake = 0,
      { betSelections, sys_bet_variant, betMode, betStake } =
        this.props.sportsbook;
    if (betMode === 1) {
      Object.keys(betSelections).forEach((sele, ind) => {
        if (
    ...
    } else if (betMode === 3 && betStake) {
      return Number.parseFloat(
        betStake * sys_bet_variant.bets || 0,
        10
      ).toFixed(2);
    } else
      return Number.parseFloat(betStake > 0 ? betStake : 0.0, 10).toFixed(2);
  }
  ...
  render() {
    ...
    let betlen =
      null !== betSelections && undefined !== betSelections
        ? Object.keys(betSelections).length
        : 0;
    this.sys_bet = [];
    Object.keys(betSelections).forEach((sele, ind) => {
      if (betSelections[sele].price >= 1.3) qualifiedSelectionCount += 1;
      newSelection.push(betSelections[sele]);
      if (min_variant < betlen) {
        let opts = 0,
          nf = this.factorial(betlen),
          rf = this.factorial(min_variant),
          nrf = this.factorial(betlen - min_variant);
        opts = nf / (rf * nrf);
        this.sys_bet.push({
          variant: min_variant,
          sys: min_variant + "/" + betlen,
          bets: opts,
        });
      }
      min_variant++;
    ...
    if (betStake > 0)
      for (const rule in sportsbettingRules) {
        if (
          ...
          if (
            qualifiedSelectionCount < betlen &&
            null !== sportsbettingRules[rule].IgnoreLowOddSelection
          )
      ...
      betlen > 0 && (
        ...
        >
          ...
          >
            {!isBetSlipOpen && (
              <div
                className="betslip-opener-container"
                  ...
                  <div className="betslip-container">
                    <div className="betslip-header-settings-container">
                      <div className="betslip-header-settings betslip-header">
                        ...
                        <div className="right">
                          <div
                            className="settings-icon-container icon"
                            onClick={this.openBetslipSettings}
                          >
                            <span
                              className={`icon-icon-settings rotate ${
                                showBetslipSettings && "down"
                              }`}
                            ></span>
                          </div>
                          <div
                      ...
                      >
                        {showBetslipSettings && (
                          <div className={`betslip-settings`}>
                            <OddsType
                              onChange={this.onOddsTypeChange}
                              value={oddType}
                            />
                            {isLoggedIn && (
                              <OddsSettings
                                onChange={this.onOddsSettingsChange}
                                value={acceptMode}
                                title={"Automatically accept odds."}
                              />
                            )}
                          </div>
                        )}
                    ...
                    <div className="betslip-body">
                              ...
                              {enableFreebet ? (
                                ...
                                >
                                  <div className="betslip-quick-bet-container open">
                                    <OddsSettings
                                      onChange={this.setFreeBetStake.bind(this)}
                                      customInput={freeBets}
                                      value={freeBetStake}
                                      title={"Choose Stake"}
                                    />
                                  </div>
                      ...
                      {betlen > 0 ? (
                        <React.Fragment>
                          <div className="betslip-tabs">
                            <div className="betslip-panel">
                              ...
                              <div
                                className="clear-all"
                                onClick={() => this.removeAllBetSelections()}
                              >
                          ...
                          <div className="betslip-matches">
                                  ...
                                  >
                                    <div className="sb-bet-body ">
                                      <div className="betslip-match-info-teams-block">
                                        ...
                                        <div
                                          className="betslip-match-right-block col-sm-2"
                                          style={{ display: "flex" }}
                                        >
                                          <div
                                            tabIndex="0"
                                            id={selection.marketId}
                                            className="betslip-match-details-block"
                                            onFocus={(e) =>
                                              this.showMatchDetails(e, {
                                                ...selection.gamePointer,
                                                title: selection.title,
                                              })
                                            }
                                            onBlur={this.hideMatchDetails}
                                          >
                                            <i
                                          ...
                                          <div
                                            className="betslip-match-remove"
                                            onClick={() =>
                                              this.removeGameFromBets(
                                                selection.marketId
                                              )
                                            }
                                          >
                                      ...
                                      <div className="sb-bet-info">
                                          ...
                                          <div className="sb-event-overlay suspended">
                                            <div className="sb-event-overlay-info suspended-container">
                                              <div className="suspended-container-part">
                                                ...
                                                <span className="sb-event-overlay-title">
                                                  Suspended
                                                </span>
                                              </div>
                                            </div>
                                        ...
                                        <div className="sb-bet-info-content">
                                          <span className="betslip-match-market-type">
                                            {selection.marketName}{" "}
                                            {selection.marketBase}
                                          </span>
                                                  ...
                                                  {oddConvert(
                                                    {
                                                      main: {
                                                        ...
                                                        notLockedOddMinValue:
                                                          null,
                                                        roundDecimalCoefficients: 3,
                                                        showCustomNameForFractionalFormat:
                                                          null,
                                          ...
                                          <div className="sb-bet-input-block stake-text keyboard-close">
                                            ...
                                            <div className="sb-input-inner-label">
                                              ...
                                              <input
                                                placeholder="0"
                                                type="number"
                                                value={selection.singleStake}
                                                onChange={(e) =>
                                                  this.setSingleBetStake(
                                                    selection.marketId,
                                                    e
                                                  )
                                                }
                                        ...
                                        <div className="sb-bet-return top-border">
                                          ...
                                          <span className="possible-win">
                                            {betMode === 3
                                              ...
                                              : betMode === 4
                                              ? Number.parseFloat(chainWinning)
                                                  .toFixed(2)
                                                  .toString()
                                                  .replace(
                                                    /\d(?=(\d{3})+\.)/g,
                            ...
                            betMode !== 1 ? (
                              <div className="booking-id-block">
                                <div className="text">
                                  <span>
                                    {bookingNumber !== 1 ? "Booking ID: " : ""}{" "}
                                  </span>
                                  <span>
                                    {bookingNumber !== 1 ? bookingNumber : null}
                                  </span>
                                </div>
                                <div
                                  className="print-btn"
                            ...
                            <div className="betBtn-container">
                              {betFailed && isOddChange ? (
                                <button
                                  onClick={(e) => {
                                    this.onOddsSettingsChange({
                                      target: { value: 2 },
                                    });
                                    this.placeBet();
                                  }}
                                  className={`placebet ${
                                    betSlipMode !== 2 ? "betslip-hide" : ""
                                ...
                                <button
                                  ...
                                  disabled={
                                    (betSlipMode === 2 &&
                                      ...
                                      profile.bonus === "0.00" &&
                                      parseFloat(
                                        parseFloat(betStake).toPrecision(12)
                                      ) >
                                        parseFloat(profile.Balance).toPrecision(
                                          12
                                        ) &&
                                      !enableFreebet) ||
                                    (betSlipMode === 2 &&
                                      isLoggedIn &&
                                      parseFloat(
                                        parseFloat(profile.bonus).toPrecision(
                                          12
                                        )
                                      ) > 0 &&
                                      parseFloat(
                        ...
                        <div className="betslip-empty guest">
                          <span>
                            No bets have been selected. To bet, please click on
                    ...
                    >
                      {(showBetSlipNoty) =>
                        ...
                        ((props) => (
                          <BetSlipNotification
                            style={props}
                            message={betSlipNotyMsg}
                            type={betSlipNotyType}
                            canNotify={showBetSlipNoty}
                            onClose={this.hideNoty}
                          />
                        ))
                      }
                    </Transition>
          ...
          {showMatchDetails && (
            <div
              id="match-details-betslip"
              className="betslip-match-details left ember-view"
              style={{
                top: details.top + "px",
            ...
            >
              <div
                className={`${details.match.sport.toLowerCase()} sb-sport-background fill sport-header sport-border`}
              >
